...
type VolumeSnapshotDataLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.VolumeSnapshotData, err error)
	// Get retrieves the VolumeSnapshotData from the index for a given name.
	Get(name string) (*v1alpha1.VolumeSnapshotData, error)
...
// List lists all VolumeSnapshotDatas in the indexer.
func (s *volumeSnapshotDataLister) List(selector labels.Selector) (ret []*v1alpha1.VolumeSnapshotData, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.VolumeSnapshotData))
	})
	return ret, err
}

// Get retrieves the VolumeSnapshotData from the index for a given name.
func (s *volumeSnapshotDataLister) Get(name string) (*v1alpha1.VolumeSnapshotData, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("volumesnapshotdata"), name)
	}
	return obj.(*v1alpha1.VolumeSnapshotData), nil
}
